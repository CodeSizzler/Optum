---
lab:
  az204Title: "Whitepaper discussion - Construct a polyglot data solution using Azure Cosmos DB"
  az204Module: "Develop solutions that use Cosmos DB storage"
---

# Whitepaper discussion - Construct a polyglot data solution using Azure Cosmos DB
...
Find your machine has Windows 11 OS and other tools including the below:
...
1. In the **Upload blob** pop-up, perform the following actions:
   ...
   b. In the **File Explorer** window, browse to the downloaded files referencing the path - **Allfiles \\Allfiles\\Labs\\04\\Starter\\Images**, select all 42 individual **.jpg** image files, and then select **Open**.
...
1. From the **File** menu, select **Open File**, browse to **Allfiles \\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Upload**, select **models.json**, and then select **Open**.
...
1. In the **File Explorer** window that opens, browse to **Allfiles \\Allfiles\\Labs\\04\\Starter\\AdventureWorks**, and then select **Select Folder**.
...
1. From the terminal prompt, run the following command to add the Azure Cosmos DB .NET client library to the currently opened project:
   ...
   > **\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Upload\\**
...
1. On line 19, set the value of **JsonFilePath** by replacing the empty string with **"\\\\Allfiles\\\\Labs\\\\04\\\\Starter\\\\AdventureWorks\\\\AdventureWorks.Upload\\\\models.json"**.
...
1. In the **Visual Studio Code** window, right click on **AdventureWorks.Context** folder in the explorer pane, and then select **Open in Integrated Terminal**.
   ...
   > **Note**:Before you perform the next step, open Windows Explorer and remove the Read-only attribute from the file **Allfiles \Allfiles\Labs\04\Starter\AdventureWorks\AdventureWorks.Context\AdventureWorks.Context.csproj**
...
1. From the terminal prompt, verify that the current directory is set to **AdventureWorks.Context** (or change it to that if it's not), and then run the following command to import **Microsoft.Azure.Cosmos** from NuGet:
   ...
   > **Note**: Before proceeding with the following command, kindly ensure that you are presently positioned at this path:
   > **\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Context\\**
...
1. From the terminal prompt, with the current directory set to **AdventureWorks.Context**, run the following command to build the .NET web application:
   ...
   > **Note**: If there are any build errors, review the **AdventureWorksCosmosContext.cs** file in the **Allfiles    > \\Allfiles\\Labs\\04\\Solution\\AdventureWorks\\AdventureWorks.Context** folder.
...
1. From the terminal prompt, run the following command to run the ASP.NET web application:
   ...
   > **Note**: Before proceeding with the following command, kindly ensure that you are presently positioned at this path:
   > **\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Web\\**
